...
def custom_mse(lmbda):
    def custom_mse_function(y_true, y_pred):
        return tf.reduce_mean(tf.reduce_sum(lmbda * tf.math.squared_difference(y_true, y_pred), axis=0))
    return custom_mse_function


class create_diffANN(keras.Model, ABC):
    def __init__(self, n_hidden=4, n_neurons=6, activation='softplus', seed=None, **kwargs):
...
class create_ANN(keras.Model, ABC):
    def __init__(self, mean_, scale_, n_hidden=4, n_neurons=6, activation='softplus', seed=None, **kwargs):
        super().__init__(**kwargs)
        self.mean_ = mean_
        self.scale_ = scale_
        self.initializer = keras.initializers.he_normal(seed=seed)
...
class NeuralNetwork:
    ...
    def compile(self, lmbda=None, n_hidden=4, n_neurons=6, act='softplus', lr=0.1, seed=None, **kwargs):
        self.model = create_diffANN(n_hidden, n_neurons, act, seed)
        opt = keras.optimizers.Adam(learning_rate=lr)
        if lmbda is None:
            self.model.compile(loss='mse', optimizer=opt, **kwargs)
        else:
            loss_func = custom_mse(lmbda)
            self.model.compile(loss=['mse', loss_func], optimizer=opt, **kwargs)

        return self.model

    def fit(self, x, y, epochs=150, batch_size=200, early_stopping=True, **kwargs):
    ...
    def compile_fit(self, x, y, epochs=150, batch_size=200, n_hidden=4,
        ...
        lmbda = None
        if z.size > 0:
            lmbda = np.linalg.norm(y1) ** 2 / np.linalg.norm(z, axis=0) ** 2

        self.compile(lmbda, n_hidden, n_neurons, act, lr, seed, **kwargs)
